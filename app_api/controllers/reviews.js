  ...
  if(!location){
    ...
    location.save(function(err, location){
      ...
      if (err){
        ...
        updateAverageResponse(res, 400, err);
...
var updatedAverageRating = function(locationid){
...
var doSetAverageRating = function(location){
  ...
  if (location.reviews && location.reviews.length > 0){
    ...
    for (i = 0; i < reviewCount; i++){
      ratingTotal = ratingTotal + locationnn.reviews[i].rating;
    }
...
module.exports.reviewReadOne = function(req, res){
  if (req.params && req.params.locationid && req.params.reviewid){
    Loc.findById(req.params.locationid).select('name reviews').exec(function(err, locations){
      var response, review;
      if (!location){
        sendJsonResponse(res, 404, {"message": "locationid not found"});
        return;
...
module.exports.reviewCreate = function(req, res){
  ...
  if(locationid){
    ...
    sendJsonResponse(res, 404, {
      "message": "Not found, location requires"
    });
  }
};
...
module.exports.reviewsUpdateOne = function(req, res){
  ...
  Loc.findById(req.params.locationid).select("reviews").exec(function(){
    var thisReview;
    if(!location){
      sendJsonResponse(res, 404, {"message": "locationid not found"});
      return;
